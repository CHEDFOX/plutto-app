/**
 * CHART OVERVIEW SECTION — Main scroll hook.
 * Pure template. All text from backend. 7-day cache, day-6 refresh.
 * Birth chart doesn't change — long cache is safe.
 */
...
export default function ChartOverviewSection({ kundliData, language = 'en', onOpenChart, onImpulse }) {
  ...
  const fetchChart = useCallback(async () => {
    ...
    return d.overview || d.reading || d;
  }, [kundliData, language]);

  useEffect(() => {
    if (!kundliData) return;
    const bd = kundliData?.raw?.birth_details || {};
    const cacheKey = `chart_overview_${bd.year}_${bd.month}_${bd.day}`;
      ...
      try {
        ...
        if (cached) {
          ...
          if (cd?.hook_title && age < CACHE_TTL) {
            setData(cd);
            if (age > REFRESH_AT) {
              fetchChart().then(fresh => { if (fresh?.hook_title) { setData(fresh); AsyncStorage.setItem(cacheKey, JSON.stringify({ data: fresh, ts: Date.now() })); } }).catch(() => {});
            }
            return;
          }
        }
        ...
        setData(fresh);
        if (fresh?.hook_title) AsyncStorage.setItem(cacheKey, JSON.stringify({ data: fresh, ts: Date.now() }));
      } catch (e) {
        try { const fresh = await fetchChart(); setData(fresh); } catch (_) {}
      }
    })();
  }, [kundliData]);
