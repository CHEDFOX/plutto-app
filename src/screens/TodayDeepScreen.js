...
// Media slot — tries to load from backend. Shows nothing if file doesn't exist.
function MediaSlot({section,system,slot,height}){
  const[ok,setOk]=useState(false);
  const path=`readings/${section}/${system}/slot_${slot}`;
  // Try common extensions
  const exts=['.jpg','.png','.mp4','.gif'];
  const[tryIdx,setTryIdx]=useState(0);
  const currentPath=path+exts[tryIdx];
  return<View style={{width:'100%',height:ok?height:0,marginVertical:ok?24:0}}>
    <MediaView uri={currentPath} style={{width:'100%',height}} onLoaded={()=>setOk(true)} onFailed={()=>{if(tryIdx<exts.length-1)setTryIdx(tryIdx+1)}}/>
  </View>
}
...
export default function TodayDeepScreen({visible,onClose,todayData,onImpulse}){
  ...
  const csR=useRef(0),scRef=useRef(null),tmr=useRef(null);
  ...
  const finishTrans=useCallback(()=>{if(tmr.current){clearTimeout(tmr.current);tmr.current=null}const n=(csR.current+1)%SYS.length;setCs(n);csR.current=n;setTrans(false);setVidSrc(pick());scRef.current?.scrollTo({y:0,animated:false});cF.setValue(0);Animated.timing(cF,{toValue:1,duration:600,easing:Easing.bezier(.25,.1,.25,1),useNativeDriver:true}).start()},[]);
  const trigLens=useCallback(()=>{if(trans)return;onImpulse?.();Animated.timing(cF,{toValue:0,duration:300,useNativeDriver:true}).start(()=>{scRef.current?.scrollTo({y:0,animated:false});setTrans(true);tmr.current=setTimeout(finishTrans,4500)})},[trans,onImpulse]);
      ...
      <ScrollView ref={scRef} showsVerticalScrollIndicator={false} contentContainerStyle={ms.sC} bounces={false} scrollEnabled={!trans}>
        ...
        {!trans?<Animated.View style={{opacity:cF}}>
          {(()=>{const sk=SYS[cs],sd=rd[sk];if(!sd)return null;
            ...
            const IMG_H=[160,130,180,140,150];
            return<View style={ms.rW}>
              ...
              {si===0?<Reveal visible={true} delay={sa?0:200} sa={sa}><MediaSlot section="today" system={sk} slot={0} height={IMG_H[si]}/></Reveal>:null}
              {si===4?<Reveal visible={true} delay={sa?0:200} sa={sa}><MediaSlot section="today" system={sk} slot={0} height={IMG_H[si]}/></Reveal>:null}
              {sd.reading?<Reveal visible={true} delay={sa?0:300} sa={sa}><Text style={ms.reading}>{sd.reading}</Text></Reveal>:null}
              {si===1&&lv>=1?<Reveal visible={true} sa={sa}><MediaSlot section="today" system={sk} slot={1} height={IMG_H[si]}/></Reveal>:null}
              ...
              {si===2&&lv>=1?<Reveal visible={true} sa={sa}><MediaSlot section="today" system={sk} slot={2} height={IMG_H[si]}/></Reveal>:null}
              ...
              {si===3&&lv>=2?<Reveal visible={true} sa={sa}><MediaSlot section="today" system={sk} slot={3} height={IMG_H[si]}/></Reveal>:null}
