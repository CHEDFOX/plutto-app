...
const SYS=['vedic','kp','western','chinese','numerology'],IMG_H=[180,150,140,170,160];
...
function ImageSpace({height}){const f=useRef(new Animated.Value(0)).current;useEffect(()=>{Animated.timing(f,{toValue:1,duration:1200,delay:400,useNativeDriver:true}).start()},[]);return<Animated.View style={[ms.imgSpace,{height,opacity:f}]}/>}
    ...
    <Animated.View style={[ms.sh,{transform:[{translateY:sl}]}]}>
      ...
      <ScrollView ref={scRef} showsVerticalScrollIndicator={false} contentContainerStyle={ms.sC} bounces={false} scrollEnabled={!scrollLock}>
        <Animated.View style={{opacity:pF,transform:[{translateY:pS}]}}>
          <Animated.View style={{opacity:cO}}>
            {(()=>{const sk=SYS[cs],sd=chartData[sk];if(!sd)return null;
              ...
              return<View style={ms.rW}>
                ...
                {(si===0||si===1)?<Reveal visible={true} delay={sa?0:500} sa={sa}><ImageSpace height={IMG_H[si]}/></Reveal>:null}
                {sd.reading?<Reveal visible={true} delay={sa?0:600} sa={sa}><Text style={ms.reading}>{sd.reading}</Text></Reveal>:null}
                {si===2?<Reveal visible={true} delay={sa?0:700} sa={sa}><ImageSpace height={IMG_H[si]}/></Reveal>:null}
                ...
                {si===3&&lv>=1?<Reveal visible={true} sa={sa}><ImageSpace height={IMG_H[si]}/></Reveal>:null}
                ...
                {si===4&&lv>=2?<Reveal visible={true} sa={sa}><ImageSpace height={IMG_H[si]}/></Reveal>:null}
          ...
          {(()=>{const sk=SYS[cs],sd=chartData[sk],lv=exp[sk]||0,isL=cs===SYS.length-1;
